...
struct TerrainGeneratorBase
{
    ...
    TerrainGeneratorBase()
    {
        ...
        std::srand(PlatformTime::GetCPUCounter());
    }
    ...
    {
        return perlinNoise.sample3D(x * freq, 0, z * freq);
    }

    float Sample3D(int x, int y, int z, float freq)
    {
        return perlinNoise.sample3D(x * freq, y * freq, z * freq);
    }

    BlockState GetOreBlock(int y)
    {
        // diamonds
        if (y < 16)
        {
